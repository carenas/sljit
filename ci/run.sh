...
if [ "$ID" = "fedora" ]; then
  ...
  qemu-aarch64-static -L /usr/aarch64-linux-gnu bin/sljit_test

  make clean
  make CROSS_COMPILER=arm-linux-gnueabihf-gcc sljit_test
  qemu-arm-static -L /usr/arm-linux-gnueabihf bin/sljit_test
  ...
  if [ $VERSION_ID -ne 19 ]; then
    ...
    qemu-mips64el-static -L /usr/mips64el-linux-gnuabi64 bin/sljit_test
...
# segfaults in debian 9 and ubuntu 18.04
    if [[ ("$ID" = "ubuntu" && ( $VERSION_ID -eq 16 || $VERSION_ID -eq 20 )) ||
      ...
      qemu-mips-static -L /usr/mips-linux-gnu bin/sljit_test
    fi
  fi
  ...
  qemu-sparc32plus-static -L /usr/sparc64-linux-gnu bin/sljit_test
...
# fails test54 case 19 in ubuntu 19 and debian stable because of NaN bug
  if [[ ("$ID" = "ubuntu" && $VERSION_ID -eq 16) ||
    ...
    qemu-ppc64le-static -L /usr/powerpc64le-linux-gnu bin/sljit_test
  fi
...
# fails test54 case 18 in debian 10 and ubuntu 18.04 thru 19.10 because of NaN
  if [[ ($VERSION_ID -eq 16 || $VERSION_ID -ge 20) &&
        "$ID" = "ubuntu" ]]; then
    qemu-ppc-static -L /usr/powerpc-linux-gnu bin/sljit_test
  fi
fi
